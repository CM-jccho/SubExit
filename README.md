...
**상황을 대화로 정리 → 대화 카드 저장 → 검색·재사용 → 내 맥락에 맞는 코칭.**

홈은 저장한 대화 중심입니다. 고정 시나리오와 XP는 메인에서 제외하고 문장 연습을 `/practice`로 이동했습니다. 상대/내 역할/상황/목표/지킬 선/말투를 자유롭게 정하고 수정·복사·삭제할 수 있습니다. 저장한 맥락은 실제 코칭 API에 전달됩니다.

카드는 이 기기의 브라우저에만 저장되며 JSON으로 내려받을 수 있습니다. 설정 대화·음성·통화 원문은 저장하지 않습니다. 로그인·기기 간 동기화는 구현하지 않았습니다.

API 키가 없어도 질문 안내와 직접 편집으로 카드를 만들 수 있습니다. 실제 AI 정리는 Gemini 연결과 전송 확인 후 사용할 수 있습니다. AI가 만든 초안은 저장 전에 직접 확인합니다.

[이번 재구성의 판단·화면·데이터 모델·시연안](docs/submission/07-conversation-cards.md)
