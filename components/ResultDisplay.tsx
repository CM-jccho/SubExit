...
function SpeakButton({ text }: { text: string }) {
  ...
  return (
    <button
      ...
      className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg bg-primary-500/10 hover:bg-primary-500/20 border border-primary-500/30 text-xs font-semibold text-primary-400 transition-all active:scale-95"
      aria-label={isSpeaking ? '음성 정지' : '음성 읽기'}
    >
      {isSpeaking ? '⏸' : '🔊'}
      <span>{isSpeaking ? '정지' : '듣기'}</span>
    </button>
  )
}
                  ...
                  <div key={i} className="bg-white/5 rounded-2xl p-4 border border-white/10">
                    ...
                    <div>
                      <div className="flex items-center justify-between mb-2">
                        <div className="text-xs font-bold text-primary-400 uppercase tracking-wider">응답 예시</div>
                        <SpeakButton text={script.sayKo} />
                      </div>
