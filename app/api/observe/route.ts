...
export async function POST(request: Request) {
  try {
    ...
    if (e instanceof Error && e.message === "unsupported_policy_claim")
      return json(
        {
          code: "observer_policy_claim",
          error:
            "AI 대본이 확인되지 않은 기관 규칙을 단정해 표시를 멈췄어요. 작성 샘플을 보거나 대화를 다시 만들어 주세요.",
        },
        502,
      );
    return apiError(e);
