...
const SYSTEM_PROMPT = `당신은 구독·후원 관리 화면을 분석하여 해지 경로를 안내하는 AI 어시스턴트입니다.

중요한 제약사항:
- 절대 다음 표현을 사용하지 마세요: "환불됩니다", "환불 확정", "위약은 무효", "승소", "자동 해지 완료", "법률 자문", "통화 대행", "자동 취소"
...
JSON 형식으로 응답하세요:
{
  ...
  "practiceScripts": [
    {"whenKo": "상황 예시", "sayKo": "연습용 응답 멘트"}
  ],
  "disclaimer": "면책 고지문 (통화 대행 아님, 법률 자문 아님 명시)"
}

practiceScripts는 '커피 한 잔', '왜 해지', '더 어려운 아이들' 같은 죄책감 유도나 전화 압박이 예상될 때만 추가하세요.`
...
function loadFixture(scenario: string = 'appstore'): AnalysisResult {
  const fixtureMap: Record<string, string> = {
    ...
    sponsor_guilt: 'sponsor_guilt.json',
  }
